#!/usr/bin/env python3
"""
REGULAR WEBSITE PIPELINE - For Standard Funder Websites
========================================================

This pipeline processes foundations with regular websites (not Charity Commission).
Key characteristics:
- Processes foundations from CSV file
- Documents are optional - can skip if they fail/timeout
- Standard Docling timeout (5 minutes)
- Focuses on website content and embedded documents
- Change detection enabled for efficiency
...
async def extract_charity_commission_documents(crawler: AsyncWebCrawler, base_url: str, charity_number: str) -> List[str]:
    ...
    # Use longer wait for JavaScript-rendered content
    crawler_config = CrawlerRunConfig(
        wait_until="networkidle",  # Wait for network to be idle (JavaScript loaded)
        ...
        try:
            ...
            if result.success and result.html:
                ...
                download_patterns = [
                    ...
                    r'href=["\']([^"\']*p_p_resource_id=[^"\']*resource[^"\']*)["\']',  # More general pattern
                    r'href=["\']([^"\']*\.pdf[^"\']*)["\']',
                    # Also check for download links in the links structure
                ]
                ...
                # Also check the links structure from crawl4ai (may have JS-loaded links)
                if result.links:
                    internal_links = result.links.get('internal', [])
                    for link_obj in internal_links:
                        href = link_obj.get('href', '')
                        if href and ('p_p_resource_id' in href or '.pdf' in href.lower() or 'download' in href.lower()):
                            full_url = urljoin(doc_page_url, href)
                            if full_url not in documents_to_process:
                                documents_to_process.append(full_url)
                
                page_doc_count = len([d for d in documents_to_process if doc_page_url.split('/')[-1].split('?')[0] in d])
                logger.info(f"    ✓ Found {page_doc_count} documents on {doc_page_url.split('/')[-1].split('?')[0]}")
        except Exception as e:
...
async def main(no_db_mode: bool = False):
    ...
    # Load foundation URLs from CSV (regular websites only)
    foundations = []
    if TEST_MODE:
    ...
    if not foundations:
        logger.error("No foundations found to process. Exiting.")
        return
    ...
    if CHANGE_DETECTION_ENABLED and stats['skipped'] > 0:
        savings = (stats['skipped'] / total_foundations) * 100
